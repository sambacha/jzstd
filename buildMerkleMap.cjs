...
const utils = require('./loadMerkleDistFile.cjs');
...
function render(distribution) {
  ...
  if (distribution.mode === 'diff') {
    total = ethers.BigNumber.from(distribution.info2.totals[utils.EulTokenAddr])
      .sub(distribution.info1.totals[utils.EulTokenAddr])
      .toString();
  } else {
    total = distribution.info.totals[utils.EulTokenAddr];
  }

  let output = `## OpenMEV Merkle Drop Summary\n`;

  if (distribution.mode === 'diff') {
    output += `
* Block range: ${distribution.info1.lastBlock} - ${distribution.info2.lastBlock}
* Total: \`${ethers.utils.formatEther(total)} Amount\`
...
`;
  ...
  for (let a of addrs) {
    ...
    let links = `[etherscan](https://etherscan.io/address/${a[0]}) [${a[0]})`;
    output += `| \`${a[0]}\` | \`${ethers.utils.formatEther(a[2])} AMOUNT\` | \`${percent.toFixed(
      3,
    )}%\` | ${links} |\n`;
  }
...
function diff(d1, d2) {
  ...
  for (let v of d2.values) {
    ...
    o.values.push([v[0], utils.EulTokenAddr, delta]);
  }
